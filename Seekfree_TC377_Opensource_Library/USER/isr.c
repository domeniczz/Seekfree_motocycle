...
#include "icm20602.h"
uint8 COUNT_2ms=0;
uint8 Record_Angle=0;
uint8 Record_Dis=0;
float Distance=0;
float SumAngle=0;
extern uint8  Run;
extern  uint8 Chuku_flag;
//PIT中断函数  示例
...
//   get_icm20602_gyro();
  ...
  //  DifferControl();
   ...
   Duoji_Work();
	...
	//记录距离与路程
//    if(Record_Dis){
//    Distance=Distance+V_Avg*0.005;
//    }
//    if(Record_Angle){
//      SumAngle=SumAngle+icm_gyro_z*0.001;
//    }

    if(Run){
//     if(!Chuku_flag){
	    Angle_adjust_out();
         Dianji_Out(Angle_duty,0);
       }
//       else{
//         Record_Angle=1;
//         if((SumAngle>600&&Zerbra_Dir==L_go)||(SumAngle<-600&&Zerbra_Dir==R_go)) {
//           Chuku_flag=0;
//           SumAngle=0;
//           Record_Angle=0;
//          }
